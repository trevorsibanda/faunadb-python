...
class DeserializationTest(TestCase):
  ...
  def test_set_ref(self):
    self.assertJson('{"@set":{"match":{"@ref":"classes/widgets"},"terms":"Laptop"}}', SetRef({"match": Ref("classes/widgets"), "terms": "Laptop"}));

  def test_fauna_time(self):
    self.assertJson('{"@ts":"1970-01-01T00:00:00.123456789Z"}', FaunaTime('1970-01-01T00:00:00.123456789Z'))
  ...
  def test_array(self):
    ...
    self.assertJson('[{"@ref":"classes/widgets"},{"@date":"1970-01-01"}]', [Ref("classes/widgets"), parse_date("1970-01-01").date()])
